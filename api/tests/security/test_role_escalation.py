...
def test_user_cannot_self_escalate_via_user_info_update(authenticated_user, test_user, db_session):
    ...
    assert row[0] == "user", f"User role was escalated to {row[0]} via info update"


@pytest.mark.tier0
@pytest.mark.security
def test_user_cannot_escalate_via_settings(authenticated_user, test_user, db_session):
    ...
    if row:
        assert row[0] == "user", f"User role was escalated to {row[0]} via settings"


@pytest.mark.tier0
@pytest.mark.security
def test_user_cannot_escalate_via_profile_update(authenticated_user, test_user, db_session):
    ...
    if row:
        assert row[0] == "user", f"User role was escalated to {row[0]} via profile update"
