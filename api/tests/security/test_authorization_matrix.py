...
def test_anonymous_cannot_access_admin_endpoint(client, method, endpoint):
    ...
    assert resp.status_code in (401, 403), (
        f"Anonymous {method} {endpoint} returned {resp.status_code}, " f"expected 401 or 403"
    )
...
def test_user_cannot_access_admin_endpoint(authenticated_user, method, endpoint):
    ...
    assert resp.status_code in (401, 403), (
        f"User {method} {endpoint} returned {resp.status_code}, " f"expected 401 or 403"
    )
...
def test_user_cannot_delete_other_user_chat(authenticated_user, db_session):
    ...
    row = db_session.execute(text("SELECT id FROM chat WHERE id = :id"), {"id": chat_id}).fetchone()
    assert row is not None, "User A's DELETE actually removed user B's chat — horizontal " "isolation is broken!"
...
def test_user_cannot_list_other_users_chats(authenticated_user, db_session, test_user):
    ...
    if resp.status_code == 200:
        ...
        assert "User B's Private Chat" not in titles, "User A's chat list includes user B's chats"
