...
def test_training_job_accepts_all_canonical_states(db_session, test_admin):
    ...
    for state in CANONICAL_STATES:
        ...
        assert job.status == state, f"Training model rejected canonical state {state!r}"
...
@pytest.mark.tier1
def test_reject_training_job_writes_rejection_marker(authenticated_admin, db_session, test_admin):
    ...
    body = resp.json()
    assert body["status"] == "cancelled", f"Rejected job should be in cancelled state, got {body['status']!r}"
...
@pytest.mark.tier1
def test_cancel_training_job_does_not_set_rejection_marker(authenticated_admin, db_session, test_admin):
    ...
    assert "reject" not in marker.lower(), (
        f"User-initiated cancel should NOT carry a rejection marker; " f"error_message was {marker!r}"
    )


@pytest.mark.tier1
def test_reject_scheduled_job_allowed_per_kit(authenticated_admin, db_session, test_admin):
...
@pytest.mark.tier1
def test_cancel_already_cancelled_is_2xx_noop(authenticated_admin, db_session, test_admin):
    ...
    assert 200 <= resp.status_code < 300, (
        f"Cancel-on-cancelled must be 2xx no-op; got {resp.status_code}: " f"{resp.text[:200]}"
    )


@pytest.mark.tier1
def test_approve_queued_job_returns_4xx(authenticated_admin, db_session, test_admin):
    ...
    assert 400 <= resp.status_code < 500, f"Approve-on-queued must be 4xx; got {resp.status_code}"


@pytest.mark.tier1
def test_approve_running_job_returns_4xx(authenticated_admin, db_session, test_admin):
...
@pytest.mark.tier1
def test_approve_completed_job_returns_4xx(authenticated_admin, db_session, test_admin):
...
@pytest.mark.tier1
def test_cancel_completed_job_returns_4xx(authenticated_admin, db_session, test_admin):
...
@pytest.mark.tier1
def test_cancel_failed_job_returns_4xx(authenticated_admin, db_session, test_admin):
...
@pytest.mark.tier1
def test_approve_pending_job_illegal_state_check(authenticated_admin, db_session, test_admin, test_app):
    ...
    if resp.status_code == 400:
        ...
        assert (
            "only pending or scheduled" not in body_lower
...
@pytest.mark.tier1
def test_cancel_running_job_propagates_to_llamolotl(authenticated_admin, db_session, test_admin, test_app):
    ...
    with aioresponses() as m:
        ...
        assert len(m.requests) > 0, "Expected at least one upstream request to llamolotl DELETE endpoint"
...
def test_cancel_local_transitions_regardless_of_upstream_response(
    ...
    assert resp.status_code == 200, f"Local cancel should succeed despite upstream 503; got {resp.status_code}"
    assert resp.json()["status"] == "cancelled"
...
@pytest.mark.tier1
def test_sync_does_not_resurrect_cancelled_job(authenticated_admin, db_session, test_admin, test_app):
    ...
    assert resp.status_code == 200
    assert resp.json()["status"] == "cancelled", (
        f"Local cancelled must not be overwritten by upstream 'running'; " f"got {resp.json()['status']!r}"
    )
...
@pytest.mark.tier1
def test_sync_advances_non_terminal_job_to_upstream_status(authenticated_admin, db_session, test_admin, test_app):
...
@pytest.mark.tier1
def test_sync_upstream_5xx_does_not_crash(authenticated_admin, db_session, test_admin, test_app):
    ...
    assert resp.status_code >= 400, f"Upstream error should surface as 4xx/5xx; got {resp.status_code}"
    assert resp.status_code != 500, (
        f"Router must handle the error, not crash (500 == unhandled). " f"Got {resp.status_code}: {resp.text[:200]}"
    )
    ...
    assert refetch.json()["status"] == "running", "Local state must not be mutated when upstream errors"
...
def test_approve_from_illegal_state_returns_4xx_no_mutation(
    ...
    assert _fetch_status(job_type, job.id) == from_state, f"{job_type} approve from {from_state!r} mutated state"
...
def test_reject_from_illegal_state_returns_4xx_no_mutation(
    ...
    assert 400 <= resp.status_code < 500, f"{job_type} reject from {from_state!r}: expected 4xx, got {resp.status_code}"
    assert _fetch_status(job_type, job.id) == from_state, f"{job_type} reject from {from_state!r} mutated state"
...
def test_schedule_from_illegal_state_returns_4xx_no_mutation(
    ...
    assert _fetch_status(job_type, job.id) == from_state, f"{job_type} schedule from {from_state!r} mutated state"
...
def test_unschedule_from_illegal_state_returns_4xx_no_mutation(
    ...
    assert _fetch_status(job_type, job.id) == from_state, f"{job_type} unschedule from {from_state!r} mutated state"
