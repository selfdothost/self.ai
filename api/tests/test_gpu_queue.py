...
def test_fits_unknown_benchmark(db_session):
    ...
    assert _fits_in_window("unknown-bench", "language-eval", remaining_minutes=60, min_remaining_minutes=0) is True
...
def _make_eval_job(db_session, *, eval_type="language-eval", status="queued", priority="normal",
...
def test_dispatch_window_pass2_preferred_type_first(db_session):
    ...
    eval_job = _make_eval_job(db_session, eval_type="language-eval", priority="normal", status="queued",
                              created_at=base)
