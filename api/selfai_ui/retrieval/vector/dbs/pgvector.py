...
class PgvectorClient:
    ...
    def check_vector_length(self) -> None:
        ...
        """
        # On a fresh database the table doesn't exist yet (__init__ creates it
        # right after this call). reflect(only=[...]) RAISES on a missing table
        # rather than leaving metadata.tables empty, so the else-branch below is
        # unreachable in that case — guard with the inspector first.
        if not inspect(self.session.bind).has_table("document_chunk"):
            return
