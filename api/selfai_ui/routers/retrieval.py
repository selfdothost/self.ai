...
def _persist_job(job_state: dict) -> None:
    ...
    try:
        ...
        data["pages"] = [
            {"url": p["url"], "title": p.get("title", ""), "content": ""} for p in job_state.get("pages", [])
        ]
    ...
    if reranking_model:
        if any(model in reranking_model for model in ["jinaai/jina-colbert-v2"]):
            ...
            try:
                ...
                # The API-only (torch-free) image ships without sentence-transformers.
                # Surface a clear, actionable error instead of a raw ImportError.
                raise Exception(
                    ERROR_MESSAGES.DEFAULT(
                        "Local reranking requires the full image (bundles torch / "
                        "sentence-transformers). The API-only image does not include it — "
                        "leave RAG_RERANKING_MODEL empty, or run the full image."
                    )
                )
...
async def update_rag_config(request: Request, form_data: ConfigUpdateForm, user=Depends(get_admin_user)):
    ...
    if form_data.web is not None:
        request.app.state.config.ENABLE_RAG_WEB_LOADER_SSL_VERIFICATION = (
            # Note: When UI "Bypass SSL verification for Websites"=True then
...
def _sync_process_page_to_knowledge(request, page: dict, collection_name: str, user_id: str) -> str:
    ...
    process_file(
        request,
        ProcessFileForm(file_id=file_id, content=markdown, collection_name=collection_name),
    )
...
async def _embed_one_page(request, job_state: dict, collection_name: str, user_id: str, page: dict) -> None:
    ...
    try:
        await asyncio.to_thread(_sync_process_page_to_knowledge, request, page, collection_name, user_id)
        job_state["saved_count"] = job_state.get("saved_count", 0) + 1
        total = len(job_state.get("pages", []))
        log.info(
            f"_embed_one_page: saved '{page.get('title', page.get('url', '?'))}' ({job_state['saved_count']}/{total})"
        )
    except Exception as e:
        log.error(f"Failed to save page {page.get('url')}: {e}", exc_info=True)
        job_state.setdefault("save_errors", []).append({"url": page.get("url"), "error": str(e)})


async def _flush_embed_cache(
    request,
    job_state: dict,
    collection_name: str,
    user_id: str,
    ...
    while pending >= batch_size or (force and pending > 0):
        ...
        log.info(
            f"_flush_embed_cache: embedding batch of {len(batch)} "
            f"(pages {cursor+1}–{cursor+len(batch)} of {len(pages)})"
        )
        for page in batch:
...
async def _run_crawl_background(
    ...
    try:
        ...
        _persist_job(job_state)
        log.info(
            f"_run_crawl_background: job {job_state['job_id']} completed, {job_state.get('saved_count', 0)} pages saved"
        )
    except Exception as e:
        log.error(
            f"_run_crawl_background: job {job_state['job_id']} failed: {e}",
            exc_info=True,
        )
...
async def _resume_crawl_background(app_state, job_state: dict, saved: dict) -> None:
    ...
    try:
        ...
        if collection_name and user_id:
            ...
            log.warning(
                f"_resume_crawl_background: job {job_state['job_id']} missing "
                "collection_name or user_id, skipping embedding"
            )
            await loader.crawl_with_progress(
                job_state,
                poll_interval=saved.get("poll_interval", 2),
        ...
        _persist_job(job_state)
        log.info(
            f"_resume_crawl_background: job {job_state['job_id']} completed, "
            f"{job_state.get('saved_count', 0)} pages embedded"
        )
    except Exception as e:
        log.error(
            f"_resume_crawl_background: job {job_state['job_id']} failed: {e}",
            exc_info=True,
        )
        job_state["status"] = "failed"
        job_state["error"] = str(e)
